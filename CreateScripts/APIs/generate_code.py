...
# ---------------------------
# DynamoDB setup (not used in orchestrator directly but kept for parity)
# ---------------------------
TABLE_NAME = "TaskBin"
...
# ---------------------------
def create_api(lambda_name="TaskBin_GenerateCode", api_name=None, account_id=None):
    """
    Creates an AWS HTTP API Gateway endpoint and integrates it with the GenerateCode Lambda.
    ...
    route_resp = apigw.create_route(
        ApiId=api_id,
        RouteKey="POST /generateCode",
        Target=f"integrations/{integration_id}"
    )

    # 4️⃣ Add Lambda permission for API Gateway
    ...
    try:
        lambda_client.add_permission(
            ...
            SourceArn=(
                f"arn:aws:execute-api:{apigw.meta.region_name}:{account_id}:{api_id}"
                "/*/POST/generateCode"
            )
        )
    ...
    endpoint = (
        f"https://{api_id}.execute-api.{apigw.meta.region_name}.amazonaws.com/dev/generateCode"
    )
